...
{
	zjQuery('#baseball_fields .show_angle').html(fields[current].angle);
	zjQuery('#baseball_fields .show_width').html(fields[current].width);
	zjQuery('#baseball_fields .show_length').html(fields[current].length);
}

function baseballSaveField()
{
	if (current != 0) {
		fields[current].angle = parseInt(zjQuery('#baseball_fields .show_angle').html());
		fields[current].width = parseInt(zjQuery('#baseball_fields .show_width').html());
		fields[current].length = parseInt(zjQuery('#baseball_fields .show_length').html());
	}
}
