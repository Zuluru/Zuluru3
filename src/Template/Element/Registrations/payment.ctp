...
if ($result === true) {
	...
	echo $this->Html->para(null, 'Alternately, you can ' .
		$this->Html->link('return to the checkout page', Router::url(['plugin' => false, 'controller' => 'Registrations', 'action' => 'checkout'], true), ['onclick' => 'close_and_redirect("' . Router::url(['controller' => 'Registrations', 'action' => 'checkout'], true) . '")']) .
