...
?>
				...
				<td class="splash_item"><?php
					...
					if ($item->home_team_id === null) {
						...
						if (in_array($item->home_team->id, $team_ids) && $item->home_team->track_attendance && !$item->game_slot->game_date->isPast()) {
					...
					if ($item->division->schedule_type != 'competition') {
						...
						if ($item->away_team_id === null) {
							...
							echo $this->element('Teams/block', ['team' => $item->away_team, 'options' => ['max_length' => 16]]) .
								' (' . __('away') . ')';
							if (in_array($item->away_team->id, $team_ids) && $item->away_team->track_attendance && !$item->game_slot->game_date->isPast()) {
								echo $this->Html->iconLink('attendance_24.png',
				...
				<td class="actions splash_item"><?php
					if (!empty($item->attendances) && in_array($item->home_team->id, $team_ids) && $item->home_team->track_attendance) {
					...
					if (!empty($item->attendances) && in_array($item->away_team->id, $team_ids) && $item->away_team->track_attendance) {
					...
					if (!empty($item->attendances) && in_array($item->home_team->id, $team_ids) && $item->home_team->track_attendance) {
						...
						if (!empty($roster) && $roster->_matchingData['TeamsPeople']->status == ROSTER_APPROVED &&
							...
							echo $this->element('Games/attendance_change', [
								...
								'status' => $record ? $record->status : ATTENDANCE_UNKNOWN,
								'comment' => $record ? $record->comment : null,
								'is_captain' => in_array($item->home_team->id, $this->UserCache->read('OwnedTeamIDs')),
								'future_only' => false,
								'dedicated' => true,
							]);
						}
					...
					if (!empty($item->attendances) && in_array($item->away_team->id, $team_ids) && $item->away_team->track_attendance) {
						...
						if (!empty($roster) && $roster->_matchingData['TeamsPeople']->status == ROSTER_APPROVED &&
							...
							echo $this->element('Games/attendance_change', [
								...
								'status' => $record ? $record->status : ATTENDANCE_UNKNOWN,
								'comment' => $record ? $record->comment : null,
								'is_captain' => in_array($item->away_team->id, $this->UserCache->read('OwnedTeamIDs')),
								'future_only' => false,
								'dedicated' => true,
							]);
						}
					...
					if (!empty($item->attendances) && $item->team->track_attendance) {
						...
						if (!empty($roster) && $roster->_matchingData['TeamsPeople']->status == ROSTER_APPROVED && $record) {
							echo $this->element('TeamEvents/attendance_change', [
								...
								'person_id' => $id,
								'role' => $roster->_matchingData['TeamsPeople']->role,
								'status' => $item->attendances[$id]->status,
								'comment' => $item->attendances[$id]->comment,
					...
					if (!empty($item->attendances) && $item->team->track_attendance) {
						...
						if (!empty($roster) && $roster->_matchingData['TeamsPeople']->status == ROSTER_APPROVED && $record) {
							echo $this->element('TeamEvents/attendance_change', [
								...
								'person_id' => $relative->id,
								'role' => $roster->_matchingData['TeamsPeople']->role,
								'status' => $item->attendances[$relative->id]->status,
								'comment' => $item->attendances[$relative->id]->comment,
								'is_captain' => in_array($item->team->id, $this->UserCache->read('OwnedTeamIDs')),
								'dedicated' => true,
							]);
						}
...
?>
				<td></td>
<?php
	elseif (is_a($item, 'App\Model\Entity\Task')):
?>
				<td class="splash_item"><?php
