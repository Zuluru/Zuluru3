...
trait CanRegister {
	 ...
	 */
	public function canRegister($person_id, $event, $price, $options = []) {
		$options += ['ignore_date' => false, 'strict' => true, 'waiting' => false, 'all_rules' => false, 'for_edit' => false];
		$notices = $this->_testCanRegister($person_id, $event, $price, $options);

		// May need to copy canRegister info from the provided price into the main list
		if ($price && (!empty($event->prices))) {
			collection($event->prices)->firstMatch(['id' => $price->id])->canRegister = $price->canRegister;
		}

		$allowed = collection($notices)->some(function ($notice) {
			return !empty($notice['allowed']);
		});
		$redirect = collection($notices)->filter(function ($notice) {
			return !empty($notice['redirect']);
		});
		if ($redirect) {
			$redirect = current($redirect->toArray())['redirect'];
		}
		return [$notices, $allowed, $redirect];
	}

	private function _testCanRegister($person_id, $event, $price, $options = []) {
		...
		if ($options['for_edit']) {
			$this->_person->registrations = collection($this->_person->registrations)->reject(function ($registration) use ($options) {
				return $registration->id == $options['for_edit']->id;
			})->toList();
		}
		...
		$notices = [];
		if ($is_registered && !$options['for_edit']) {
		...
		if (isset($price)) {
			...
			pr($event);
			trigger_error('TODOTESTING: Should be an indexed array of prices', E_USER_WARNING);
			exit;
		}
		...
		foreach ($prices as $price) {
			...
			if (!$prereg && !$options['ignore_date']) {
				// Admins can test registration before it opens...
				$event_admin = Configure::read('Perm.is_admin') || (Configure::read('Perm.is_manager') && in_array($event->affiliate_id, UserCache::getInstance()->read('ManagedAffiliateIDs')));
				if (!$event_admin && $price->open->isFuture()) {
					$price->canRegister = [
						'allowed' => false,
			...
			if (!empty($price->register_rule)) {
				if (!$rule_obj->init($price->register_rule)) {
					...
					$price->canRegister = [
						'allowed' => $rule_obj->evaluate($event->affiliate_id, $this->_person, null, $options['strict'], false),
					];
					if ($price->canRegister['allowed']) {
						...
						if ($options['strict'] && count($prices) == 1) {
