<?php
namespace BamboraPayment\Test\TestCase\Controller;
...
/**
 * BamboraPayment\Controller\PaymentController Test Case
 *
 * @property \BamboraPayment\Controller\PaymentController $_controller
 */
class PaymentControllerTest extends ControllerTestCase {
	...
	/**
	 * Set up the mock API object to avoid talking to the Bambora servers
	 */
	public function controllerSpy($event, $controller = null) {
		parent::controllerSpy($event, $controller);

		if (isset($this->_controller)) {
			$this->_controller->api = $this->getMockBuilder('BamboraPayment\Http\API')
	...
	public function testIndexAsCaptain() {
		// Bambora sends parameters in the URL.
		$data = [
			'trnApproved' => '1',
			'trnId' => '123456',
			'authCode' => 'TEST',
		...
		$this->assertGetAsAccessOk(array_merge(['plugin' => 'BamboraPayment', 'controller' => 'Payment', 'action' => 'index'], $data), PERSON_ID_CAPTAIN);
	...
	/**
	 * Bambora redirects the actual user to the payment page, so anonymous access is not allowed
	 *
	 * @return void
	 */
	public function testIndexAsAnonymous() {
		// PayPal sends parameters in the URL.
		$this->assertGetAnonymousAccessDenied(['plugin' => 'BamboraPayment', 'controller' => 'Payment', 'action' => 'index']);
	}
